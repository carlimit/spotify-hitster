...
function Game({
  ...
  const generateCard = async () => {
    ...
    const res = await axios.get(`/api/track?genre=${genre}&minYear=${min}&maxYear=${max}`);
  ...
  const handleDragStart = useCallback((e) => {
    ...
    // Must preventDefault here to block pull-to-refresh on mobile.
    // This only works because we attach via DOM with { passive: false } below.
    e.preventDefault();
    ...
    if (timeline) {
      ...
      if (clientY < SCROLL_ZONE && tlRect.top < 0) {
        scrollIntervalRef.current = setInterval(() => {
          ...
          if (timelineRef.current?.getBoundingClientRect().top >= 0) clearInterval(scrollIntervalRef.current);
        }, 16);
      } else if (clientY > window.innerHeight - SCROLL_ZONE && tlRect.bottom > window.innerHeight) {
        scrollIntervalRef.current = setInterval(() => {
          window.scrollBy(0, SCROLL_SPEED);
          if (timelineRef.current?.getBoundingClientRect().bottom <= window.innerHeight) clearInterval(scrollIntervalRef.current);
        }, 16);
      }
    }
  ...
  // Non-passive touchstart on the drag card — the ONLY way to block pull-to-refresh
  // React's onTouchStart is passive and cannot call preventDefault reliably
  useEffect(() => {
  ...
  const giveCoin = () => { socket.emit("give_coin", { code: roomCode }); };
  ...
  const claimRecognition = () => { setShowRecognition(false); socket.emit("claim_recognition", { code: roomCode }); };
                ...
                {!isMyTurn && !revealed && (
                  <div className="coin-slot">
                    {coinsHere - (myMyCoinHere ? 1 : 0) > 0 && (
                      <div className="coins-on-slot">{"🪙".repeat(Math.min(coinsHere - (myMyCoinHere ? 1 : 0), 5))}</div>
                    )}
                    {myMyCoinHere ? (
                ...
                {index === cards.length - 1 && !isMyTurn && !revealed && (
                  <div className="coin-slot">
                    {(() => {
                      ...
                      return <>
                        {othersCoins > 0 && <div className="coins-on-slot">{"🪙".repeat(Math.min(othersCoins, 5))}</div>}
                        {myBottomCoin ? (
